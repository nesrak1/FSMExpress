...
{
    public static AssetTypeValueField? GetByteArrayTexture(AssetsManager manager, AssetsFileInstance fileInst, AssetFileInfo tex)
    {
        var textureTemp = manager.GetTemplateBaseField(fileInst, tex);
        var image_data = textureTemp["image data"];
        if (image_data == null)
            return null;

        image_data.ValueType = AssetValueType.ByteArray;

        var m_PlatformBlob = textureTemp["m_PlatformBlob"];
        ...
        {
            baseField = textureTemp.MakeValue(fileInst.file.Reader, tex.GetAbsoluteByteOffset(fileInst.file));
        }
        return baseField;
    }
    ...
    public static void SwizzleOptIn(TextureFile texture, AssetsFile file)
    {
        ...
        if (file.Metadata.TargetPlatform == 38) // Switch
        {
